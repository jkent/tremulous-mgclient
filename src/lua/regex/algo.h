...
#define REX_VERSION "Lrexlib 2.7.1"
...
static int algf_gsub (lua_State *L) {
  ...
  if (argE.reptype == LUA_TSTRING) {
    rex_buffer_init (&BufRep, 256, L, &freelist);
    BUFFERZ_PUTREPSTRING (&BufRep, argE.funcpos, ALG_NSUB(ud));
  }
  /*------------------------------------------------------------------*/
  if (argE.maxmatch == GSUB_CONDITIONAL) {
    rex_buffer_init (&BufTemp, 1024, L, &freelist);
    pBuf = &BufTemp;
  }
  /*------------------------------------------------------------------*/
  rex_buffer_init (&BufOut, 1024, L, &freelist);
  SET_RETRY (retry, 0);
  while ((argE.maxmatch < 0 || n_match < argE.maxmatch) && st <= (int)argE.textlen) {
    ...
    if (ALG_NOMATCH (res)) {
      ...
      if (retry) {
        if (st < (int)argE.textlen) {  /* advance by 1 char (not replaced) */
          rex_buffer_addlstring (&BufOut, argE.text + st, ALG_CHARSIZE);
          st += ALG_CHARSIZE;
          retry = 0;
          continue;
        }
    ...
    if (st < from) {
      rex_buffer_addlstring (&BufOut, argE.text + st, from - st);
#ifdef ALG_PULL
      st = from;
#endif
    }
    ...
    if (argE.reptype == LUA_TSTRING) {
      ...
      while (bufferZ_next (&BufRep, &iter, &num, &str)) {
        if (str)
          rex_buffer_addlstring (pBuf, str, num);
        else if (num == 0 || ALG_SUBVALID (ud,num))
          rex_buffer_addlstring (pBuf, argE.text + ALG_BASE(st) + ALG_SUBBEG(ud,num), ALG_SUBLEN(ud,num));
      }
      curr_subst = 1;
    }
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
      ...
      rex_buffer_addlstring (pBuf, argE.text + from, to - from);
    }
    /*----------------------------------------------------------------*/
    if (argE.reptype == LUA_TTABLE || argE.reptype == LUA_TFUNCTION) {
      if (lua_tostring (L, -1)) {
        rex_buffer_addvalue (pBuf, -1);
        curr_subst = 1;
      }
    ...
    if (argE.maxmatch == GSUB_CONDITIONAL) {
      ...
      if (lua_isstring (L, -2)) {               /* coercion is allowed here */
        rex_buffer_addvalue (&BufOut, -2);          /* rep2 */
        curr_subst = 1;
      }
      else if (lua_toboolean (L, -2))
        rex_buffer_addbuffer (&BufOut, &BufTemp);   /* rep1 */
      else {
        rex_buffer_addlstring (&BufOut, argE.text + from, to - from); /* "no" */
        curr_subst = 0;
      }
  ...
  /*------------------------------------------------------------------*/
  rex_buffer_addlstring (&BufOut, argE.text + st, argE.textlen - st);
  rex_buffer_pushresult (&BufOut);
