...
void freelist_free (TFreeList *fl) {
  while (fl->top > 0)
    rex_buffer_free (fl->list[--fl->top]);
}
 ...
 *       *  rex_buffer_addvalue does not extract the value from Lua stack
 *       *  rex_buffer_pushresult does not have to be the last operation
 ...
 *          rex_buffer_addlstring and rex_buffer_addvalue must not be used
...
void rex_buffer_free (TBuffer *buf) {
  void *ud;
  lua_Alloc lalloc = lua_getallocf(buf->L, &ud);
  lalloc (buf->L, buf->arr, buf->size, 0);
}

void rex_buffer_clear (TBuffer *buf) {
  buf->top = 0;
}

void rex_buffer_pushresult (TBuffer *buf) {
  lua_pushlstring (buf->L, buf->arr, buf->top);
}

void rex_buffer_addbuffer (TBuffer *trg, TBuffer *src) {
  rex_buffer_addlstring (trg, src->arr, src->top);
}

void rex_buffer_addlstring (TBuffer *buf, const void *src, size_t sz) {
...
void rex_buffer_addvalue (TBuffer *buf, int stackpos) {
  size_t len;
  const char *p = lua_tolstring (buf->L, stackpos, &len);
  rex_buffer_addlstring (buf, p, len);
}
...
void bufferZ_addlstring (TBuffer *buf, const void *src, size_t len) {
  ...
  header[1] = len;
  rex_buffer_addlstring (buf, header, sizeof (header));
  rex_buffer_addlstring (buf, src, len);
  n = len % N_ALIGN;
  if (n) rex_buffer_addlstring (buf, NULL, N_ALIGN - n);
}
